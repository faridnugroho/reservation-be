...
func GetUsers(param dto.FindParameter, preloadFields []string) (responses []models.Users, total int64, totalFiltered int64, err error) {
...
func GetUserByID(id uuid.UUID, preloadFields []string) (response models.Users, err error) {
	db := utils.BuildPreload(config.DB, preloadFields)
...
func UpdateUser(data models.Users) (models.Users, error) {
	err := config.DB.Save(&data).Error

	return data, err
}

func DeleteUser(data models.Users) error {
